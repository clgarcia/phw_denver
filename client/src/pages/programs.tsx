...
import { Calendar, Clock, Users, Star, Search, MapPin } from "lucide-react";
                ...
                {filteredPrograms.map((program) => (
                  <Link key={program.id} href={`/programs/${program.id}`}>
                    <Card className="group h-full hover-elevate cursor-pointer transition-all duration-200">
                      ...
                      <CardHeader>
                        <CardTitle className="group-hover:text-primary transition-colors" data-testid={`text-program-name-${program.id}`}>
                          {program.name}
                        </CardTitle>
