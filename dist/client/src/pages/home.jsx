...
import { ArrowRight, Calendar, Clock, MapPin, Users, Heart, Waves } from "lucide-react";
...
export default function Home() {
    ...
    var getEventImage = function (event, index) { return event.imageUrl || [activityImage1, activityImage2, heroImage, activityImage1][index % 4]; };
    return (<div className="min-h-screen flex flex-col">
      <Header />
      
      <main className="flex-1">
        <section className="relative overflow-hidden min-h-[600px] flex items-center">
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url(".concat(heroImage, ")") }}/>
        ...
        <section className="py-16 lg:py-24">
          <div className="container mx-auto px-4">
            ...
            {eventsLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[1, 2, 3, 4].map(function (i) { return (<Card key={i} className="animate-pulse">
              ...
              </div>) : upcomingEvents.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {upcomingEvents.map(function (event, index) { return (<Link key={event.id} href={"/events/".concat(event.id)}>
                    <Card className="group h-full hover-elevate cursor-pointer transition-all duration-200 overflow-hidden">
                      <div className="h-48 bg-muted overflow-hidden">
                        <img src={getEventImage(event, index)} alt={event.title} className="w-full h-full object-contain mx-auto group-hover:scale-105 transition-transform duration-300 bg-white" style={{ maxHeight: 192 }}/>
                      </div>
                      <CardHeader>
                      ...
                      <CardContent className="space-y-2">
                        ...
                        <div className="pt-2">
                          <Button size="sm" className="w-full bg-[#c73e1d]/90 hover:bg-[#c73e1d] border-[#c73e1d]/90" data-testid={"button-register-event-".concat(event.id)}>
                            Register Now
                          </Button>
                        </div>
                      </CardContent>
        ...
        <section className="py-16 lg:py-24">
          <div className="container mx-auto px-4">
            ...
            {programsLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[1, 2, 3, 4].map(function (i) { return (<Card key={i} className="animate-pulse">
                    <CardHeader>
                      <div className="h-6 bg-muted rounded w-3/4"/>
              ...
              </div>) : activePrograms.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {activePrograms.map(function (program, index) { return (<Card key={program.id} className="h-full hover-elevate transition-all duration-200 overflow-hidden">
                    <div className="h-48 bg-muted overflow-hidden">
                      <img src={program.imageUrl || [activityImage1, activityImage2, heroImage, activityImage1][index % 4]} alt={program.name} className="w-full h-full object-contain mx-auto" style={{ maxHeight: 192 }}/>
                    </div>
                    <CardHeader>
                      <CardTitle data-testid={"text-program-name-".concat(program.id)}>{program.name}</CardTitle>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-center justify-end pt-2">
                        <Link href={"/register?program=".concat(program.id)}>
                          <Button size="sm" className="bg-[#c73e1d]/90 hover:bg-[#c73e1d] border-[#c73e1d]/90" data-testid={"button-register-program-".concat(program.id)}>
                            Register
                          </Button>
                        </Link>
                      </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
              <Link href="/register" className="group">
                <Card className="overflow-hidden cursor-pointer group-hover:shadow-lg transition-shadow duration-200 min-h-[400px] flex flex-col h-full">
                  <div className="h-48 overflow-hidden">
                    <img src={volunteerImage} alt="Volunteer opportunity" className="w-full h-full object-cover"/>
                  </div>
                  <CardHeader>
              ...
              <Link href="/register" className="group">
                <Card className="overflow-hidden cursor-pointer group-hover:shadow-lg transition-shadow duration-200 min-h-[400px] flex flex-col h-full">
                  <div className="h-48 overflow-hidden">
                    <img src={tripCoordinatorImage} alt="Trip coordinator" className="w-full h-full object-cover"/>
                  </div>
                  <CardHeader>
